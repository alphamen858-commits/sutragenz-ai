...
export async function POST(req: Request) {
  ...
  if (!canAccessFeature(plan, "quiz-gen")) {
    return NextResponse.json({ error: "Quiz Generator needs the Pro plan. Upgrade to unlock it." }, { status: 403 });
  }
  ...
  try {
    ...
    console.error("QUIZ ERROR:", err instanceof Error ? err.message : err);
    return NextResponse.json(
      { error: err instanceof Error ? `Quiz generation failed: ${err.message}` : "Quiz generation failed." },
      { status: 502 }
    );
  }
  ...
  await prisma.usageEvent.create({ data: { userId: session.user.id, feature: "quiz-gen" } });
