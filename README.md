...
## AI Project Builder — the honest version

This generates a REAL multi-file React project (not a description of one),
shows it running live in-browser via Sandpack (the same class of in-browser
bundler that powers CodeSandbox), and lets you download it as a zip or
deploy it live to Vercel.

**What's genuinely solid:** generation and the live Sandpack preview — both
run entirely in the browser/your server, no external dependency beyond your
AI provider key.

**What to know about "Deploy to Vercel":**
- It deploys using **your own** `VERCEL_API_TOKEN` — never a shared one —
  so deployed projects land in your Vercel account, not ours.
- It wraps the AI's output in a minimal Create React App scaffold
  server-side (`src/lib/deploy/vercel.ts`) so Vercel's build step has a
  real, buildable project. This is the least battle-tested part of this
  feature — if a generated project uses something CRA can't build cleanly,
  the deploy step will surface Vercel's real build error, not a fake
  success. Treat this as a genuine v1, not a polished, edge-case-free
  pipeline.
- This is **not** a Bolt/v0/Lovable-level product — no iterative chat-based
  editing of the generated app, no multi-page routing, no backend logic.
  It's a real, working single-page-app generator with real deployment,
  which is a meaningfully large but bounded slice of what those tools do.

## Local setup

**Note:** this project uses `next-auth` v4 with Next.js 15 — a combination
that works fine in practice but that npm's strict peer-dependency checker
objects to. A `.npmrc` file in the project root (`legacy-peer-deps=true`)
handles this automatically, so plain `npm install` should just work. If
you ever see an `ERESOLVE` error mentioning `next-auth` during install
anyway, run `npm install --legacy-peer-deps` directly.
